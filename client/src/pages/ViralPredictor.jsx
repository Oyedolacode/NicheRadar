...
export default function ViralPredictor() {
    ...
    useEffect(() => {
        if (location.state?.results) { setResults(location.state.results); return }
    }, [])
    ...
    async function run() {
        ...
        const out = []
        for (const t of topics) { try { out.push(await fetchAndEnrich(t)) } catch (e) { } }
    ...
    return (
        <div style={{ padding: 22 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 18, flexWrap: 'wrap' }}>
                ...
                <button className="btn ai" onClick={run} disabled={busy}>{busy ? '⏳ Predicting…' : '🤖 Run Prediction'}</button>
                <button className="btn s" onClick={() => setInput(PRESET)}>📋 Load Preset</button>
            </div>

            {results.length === 0 && !busy && (
                <div className="empty"><div className="ei">🤖</div><h3>Viral Predictor Ready</h3>
                    <p>Enter comma-separated topics to generate viral probability scores.</p></div>
            )}
                ...
                <>
                    {alerts.length > 0 && (
                        <div style={{ background: 'rgba(255,92,53,.06)', border: '1px solid rgba(255,92,53,.25)', borderRadius: 'var(--rl)', padding: '13px 17px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 11 }}>
                            ...
                            <div>
                                <div style={{ fontFamily: 'var(--fd)', fontWeight: 700, fontSize: 13.5, color: 'var(--hot)' }}>{alerts.length} VIRAL ALERT{alerts.length > 1 ? 'S' : ''}</div>
                                <div style={{ fontSize: 11.5, color: 'var(--muted)', fontFamily: 'var(--fm)' }}>{alerts.map(r => r.kw).join(' · ')}</div>
                            </div>
                        </div>
                    )}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(256px,1fr))', gap: 12, marginBottom: 20 }}>
                        {results.map(r => {
                            const prob = viralProbability(r); const pct = Math.round(prob * 100); const pl = probLabel(prob)
                            return (
                                <div key={r.kw} style={{ background: 'var(--surface)', border: `1px solid ${pl.alert ? 'var(--hot)' : 'var(--border)'}`, borderRadius: 'var(--rl)', padding: 18, position: 'relative', boxShadow: pl.alert ? '0 0 20px rgba(255,92,53,.15)' : 'none' }}>
                                    {pl.alert && <div style={{ position: 'absolute', top: 13, right: 13, background: 'var(--hot)', color: '#fff', fontFamily: 'var(--fm)', fontSize: 8.5, fontWeight: 700, padding: '2px 7px', borderRadius: 10, animation: 'pulse .9s infinite' }}>🚨 VIRAL</div>}
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                                        {[['Velocity', fmt(r.avgVel) + '/hr', 'var(--accent)'], ['Engagement', fmtP(r.avgEng), 'var(--yellow)'], ['Viral Gap', r.vg.toFixed(2), 'var(--green)'], ['Trend', r.trend.toFixed(1), 'var(--blue)']].map(([l, v, c]) => (
            ...
            <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                    ...
                    ['📈', 'Trend Momentum', 'Recency + velocity.', '15%', 'rgba(255,215,64,.1)', 'var(--yellow)'],
