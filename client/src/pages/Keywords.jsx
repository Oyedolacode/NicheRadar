...
export default function Keywords() {
    ...
    return (
        <div style={{ padding: 22 }}>
            <div style={{ background: 'linear-gradient(135deg,rgba(0,229,204,.08),rgba(181,122,255,.05))', border: '1px solid rgba(0,229,204,.25)', borderRadius: 'var(--r3)', padding: 'var(--s6)', marginBottom: 20 }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(0,229,204,.12)', border: '1px solid rgba(0,229,204,.25)', borderRadius: 100, padding: '4px 14px', fontFamily: 'var(--fm)', fontSize: 10, color: 'var(--accent)', marginBottom: 12, fontWeight: 700 }}>
                    🔍 KEYWORD RESEARCH
                </div>
                <div style={{ fontFamily: 'var(--fd)', fontSize: 22, fontWeight: 800, marginBottom: 8 }}>Niche Discovery Engine</div>
                <div style={{ fontSize: 13, color: '#7aadc8', lineHeight: 1.6, maxWidth: 640 }}>Find high-traffic, low-competition keywords across any niche. Analyze search volume trends and advertiser interest.</div>
            </div>

            <div style={{ display: 'flex', gap: 7, maxWidth: 700, marginBottom: 24 }}>
                ...
                <button className="btn" onClick={research} disabled={busy}>{busy ? '⏳' : '🔑'} Research</button>
            </div>
                                        ...
                                        {results.map(r => {
                                            ...
                                            return (
                                                <tr key={r.kw} onClick={() => navigate('/explorer', { state: { keyword: r.kw } })}
                                                    ...
                                                    <td style={{ padding: '9px 12px', fontWeight: 600, fontSize: 12.5 }}>{r.kw}</td>
                                                    ...
                                                    <td style={{ padding: '12px' }} className="tm">{r.sat}</td>
                                                    <td style={{ padding: '12px' }} className="tm">{fmt(avgV)}</td>
                                                    <td style={{ padding: '12px' }}><span style={{ background: 'var(--elevated)', color: si.color, fontFamily: 'var(--fm)', fontWeight: 700, fontSize: 11, padding: '2px 7px', borderRadius: 4 }}>{r.opp.toFixed(1)}</span></td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>
